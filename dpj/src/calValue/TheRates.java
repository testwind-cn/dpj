...
public class TheRates {
	...
	private double set_z_pai(int num, double mult_pai) { // 获取???
		double r = 0;
		if (num < 0 || num > d3_total_Period + 1)
			return 1;
		if (data_z_R != null && num < data_z_R.length) {
	...
	private void cal_PerRate(boolean useSelfDay) { // 算月还,是按期,还是按天
		...
		double mult_pai; // 从 2 到 第 25 个 z_pai 求和
	...
	public void cal_theRates(  calValue.TheDates theDates, double rate) {
		if ( theDates == null ) {
			return;
		}
		...
		for (int x=1; x <= num; x++)
		{
			int days = theDates.getDueDays(num);
			
			fix_z_R( x, real_day_rate,days, len,false ); // $this->d2_real_day_rate, false 按期，true 按天
		}
